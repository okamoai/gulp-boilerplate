...
gulp.task('sprite', callback => {
  ...
  gulp.src(config.tasks.sprite.template[config.defaultTasks.css].base)
    .on('end', () => onEnd('base'))
  ...
  Object.keys(sprites).forEach(key => {
    ...
    const spritePath = sprites[key].path.split(path.sep).filter((dir, idx) => {
    ...
    const spriteRetinaPath = config.tasks.sprite.retina ?
      path.join(
        ...
        spritePath.split(path.sep).map(dir => {
          if (dir === spriteName) {
            return `_${spriteName}`
          }
          return dir
    ...
    const spriteData = gulp.src(spriteFiles, { base: config.tasks.sprite.path.source })
      .pipe(sprite(Object.assign({
        imgName: `sprites_${spriteName}.png`,
        cssName: `${spriteName}.css`,
        padding: 4,
        cssTemplate: data => {
          // for CRC Hash
          const filesData = spriteFiles.reduce((concat, fPath) => concat + fs.readFileSync(fPath))
          ...
          const cssExtendPath = spritePath
            ...
            .join(path.sep)
          gulp.src(config.tasks.sprite.template[config.defaultTasks.css].sprite)
          ...
          if (config.env === 'development') {
            // CSS
            gulp.src(config.tasks.sprite.template.sample.css)
              .pipe(consolidate('lodash', consolidateOption))
            ...
            // HTML
            gulp.src(config.tasks.sprite.template.sample.html)
              .pipe(consolidate('lodash', consolidateOption))
              .pipe(rename({ basename: spriteName }))
              .pipe(gulp.dest(path.join(config.tasks.sprite.path.sample, spritePath)))
    ...
    spriteData.img
      ...
      .pipe(gulpIf(config.env === 'development', gulp.dest(path.join(config.tasks.sprite.path.sample))))
  })
})
